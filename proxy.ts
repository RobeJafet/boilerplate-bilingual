...
function getLocale(request: NextRequest): string {
  ...
  if (
    nextLocaleCookie &&
    locales.includes(nextLocaleCookie.value as LocalePage)
  ) {
...
export function proxy(request: NextRequest) {
  ...
  if (pathname.startsWith("/intent/edit/mode=presentation")) {
    const presentationPath = pathname.replace(
      "/intent/edit/mode=presentation",
      "",
    );
...
export const config = {
  matcher: [
    "/((?!api/|api$|_next/|favicon.ico|studio|sitemap.xml|robots.txt|assets/|.*\\.svg$|.*\\.png$|.*\\.jpg$|.*\\.jpeg$|.*\\.gif$|.*\\.webp$|.*\\.ico$).*)",
  ],
};
